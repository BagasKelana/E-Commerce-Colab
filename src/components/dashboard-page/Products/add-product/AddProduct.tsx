  ...
  const handleReplaceFile = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      ...
      setSelectedImages((current) => {
        if (current) {
          return current.map((file, i) => (i === +index ? files[0] : file));
        }
        return current;
      });
    }
        ...
        <form onSubmit={handleOnSubmit} className="mt-4 px-8">
          <div className="flex gap-8">
            <section className="flex-1 space-y-4">
              ...
              <div className="flex flex-col gap-2" id="UPLOAD_IMAGES">
                ...
                <div>
                  ...
                  <Label className="select-none" htmlFor="uploadFile">
                    <div className="w-full h-[200px] flex flex-col items-center justify-center gap-4 bg-transparent space-x-1 border-dashed  border border-slate-500 rounded">
                      <div>
                        <span>Drag your images here or </span>
                        <strong>Browse from device</strong>
                      </div>
            ...
            <Card className="w-[300px] h-fit">
              <CardContent className="py-8 space-y-4">
                <CategoriesBox InputForm={{ formInput, setFormInput }} />
                <ToggleBox
                  key={'featured'}
